...
#img_label_dir = './ISIC2018_Task3_Training_GroundTruth/ISIC2018_Task3_Training_GroundTruth.csv'
img_dir = 'D:\桌面\健康信息工程实训\image'
data_transforms ={
...
#img_labels=pd.read_csv(img_label_dir)
dataset = torchvision.datasets.ImageFolder(img_dir, transform=trans)


BATCH_SIZE = 30                      # 超参数batch大小
EPOCH = 3                              # 总共训练轮数
save_path = "./pfa_ResNet18.pth"    # 模型权重参数保存位置
...
n_test = int(0.3 * data_length)  # take ~10% for test
test_set = torch.utils.data.Subset(dataset, range(n_test))  # take first 10%
train_set = torch.utils.data.Subset(dataset, range(n_test, data_length))  # take the rest
print(len(test_set))

train_dataloader = torch.utils.data.DataLoader(train_set, BATCH_SIZE, True, num_workers=0)
test_dataloader = torch.utils.data.DataLoader(test_set, BATCH_SIZE, False, num_workers=0)
...
net = models.resnet18(pretrained=False).to(device)                                # 实例化网络模型并送入GPU
